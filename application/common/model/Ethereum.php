 ...
 */
require_once(APP_PATH.'common/model/Json_rpc.php');
...
class Ethereum extends Json_rpc
{
    ...
    {
        if(!is_a($message, 'Ethereum_CallBalance'))
        {
    ...
    //16进制转换成10进账
    function fromWei($weiNumber,$decimals=18)
    {
        $input=substr($weiNumber, 2);
        return hexdec($input)/pow(10,$decimals);
    }
    //十进制转换成16进制
    function toWei($ethNumber,$decimals=18)
    {
        return dechex($ethNumber*pow(10,$decimals));
    }

    function personal_newAccount($psword)
    {
        return $this->ether_request(__FUNCTION__,array($psword));
    }
    function personal_unlockAccount($address,$psw,$time)
    {
        return $this->ether_request(__FUNCTION__,array($address,$psw,$time));
    }
...
class Ethereum_CallBalance
{
        private $to,$data, $nonce,$block;
        function __construct($to,$data='', $nonce=NULL)
        {
                $this->to = $to;
                $this->data = $data;
                $this->nonce = $nonce;
